...
for i=1:dN:NT
    ...
    x = sdpvar(2, NH+1); % State variables (position)
    u = sdpvar(2, NH); % Control variables (velocity)
    ...
    % 2. Reach region T between time 4.5 and 5
    T_start_T = 4.5 / dt;
    T_end_T = 5 / dt;
    for k = 1:NH
        if i+k > T_start_T && i+k < T_end_T
            constraints = [constraints, x(:, k) >= T(:, 1) & x(:, k) <= T(:, 2)];
        end
    end
    
    if cas == 3
        % 3. Avoid obstacle G for the 5 time units
        for k = 1:NH
    ...
    x_traj = [x_traj, x_opt(:,1:dN)];
    u_traj = [u_traj, u_opt(:,1:dN)];
end
toc
